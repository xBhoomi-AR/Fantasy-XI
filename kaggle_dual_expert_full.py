...
# Full 216 Per-Step Sequence Features
RAW_HIST = [
...
def exp1_band_3cls(p):
    if p <= 5: return 0      # 3-5 pts
    elif p <= 9: return 1    # 6-9 pts
    else: return 2           # 10+ pts

def overall_band_4cls(p):
    if p <= 2: return 0      # 0-2 pts (Expert 2)
    elif p <= 5: return 1    # 3-5 pts
    elif p <= 9: return 2    # 6-9 pts
    else: return 3           # 10+ pts
...
# ─────────────────────────────────────────────────────────────────────
# STAGE 1: TRAINING UPGRADED EXPERT 2 (POSITION WEIGHTED LOSS & FEATURES)
# ─────────────────────────────────────────────────────────────────────
print("\n" + "="*80)
...
# ─────────────────────────────────────────────────────────────────────
# STAGE 2: TRAINING EXPERT 1 HIGH BAND (BALANCED 3-5 & 6-9 LOSS WEIGHTING)
# ─────────────────────────────────────────────────────────────────────
print("\n" + "="*80)
...
# Balanced Class Weights: Equal weighting across 3-5 pts and 6-9 pts
...
# ─────────────────────────────────────────────────────────────────────
# FULL TEST EVALUATION (SOFT EXPECTATION INTERPOLATION — ~1.x MAE FOR ALL BANDS)
# ─────────────────────────────────────────────────────────────────────
print("\n" + "="*95)
...
# Score Centroid Anchors per Band: [3-5 pts: 3.8 | 6-9 pts: 7.2 | 10+ pts: 11.5]
...
for i in range(len(test_df)):
    if gate_prob[i] < 0.50:
        ...
        # Enhanced Minutes-Gated Expert 2 Inference
        if pred_m2[i] < 1.0 or (gw1_mins[i] == 0 and gw1_start[i] == 0):
            ...
            if pos_te[i] in [1, 2] and xgc_vals[i] >= 1.8:
                base_p = 1.0  # Defensive Clean-Sheet Loss Penalty
            combined_pred_pts[i] = 0.70 * base_p + 0.30 * np.clip(pts2[i], 0.0, 2.4)
    else:
        # SOFT EXPECTED POINT INTERPOLATION ACROSS ALL EXPERT 1 CLASSES
        probs = p_3cls[i]
        top_cls = probs.argmax()
        combined_preds_band[i] = top_cls + 1  # 1=3-5, 2=6-9, 3=10+
        
        # Continuous expected value: P(3-5)*3.8 + P(6-9)*7.2 + P(10+)*11.5
        soft_val = np.sum(probs * anchors)
        
        # Blend 65% Soft Probability Expectation + 35% Direct Neural Regression Prediction
        combined_pred_pts[i] = 0.65 * soft_val + 0.35 * max(pts_3c[i], 3.0)

# Compute Cumulative Overall Metrics
...
# Confidence Tables for Expert 1 Bands
