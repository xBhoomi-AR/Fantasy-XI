...
OBSERVATION_SIZE = SQUAD_SIZE * PLAYER_FEATURES + 3 + len(POSITIONS)


def build_observation(state) -> np.ndarray:
    ...
    obs = np.array(player_features + team_state + position_best, dtype=np.float32)
