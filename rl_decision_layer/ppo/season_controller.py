"""Runs a sequential, multi-gameweek PPO -> MILP -> XI -> captain/vice
recommendation, carrying the squad/bank/free-transfers forward from one
gameweek to the next - the actual sequential FPL-manager product, not
independent per-gameweek optimization.

Adds no new selection/transfer-optimization logic. PPOEnv already carries
state forward internally within one run (env.py: self._state = next_state
each step, and HistoricalEnv.step() already computes transfers_made and the
real free-transfer rollover from current_squad_ids each time) - this file
only adds: (a) an outer loop that prints rich, transfer-aware output per
gameweek, and (b) optional JSON persistence of {gameweek, squad_ids, bank,
free_transfers} so a run can be resumed across separate process invocations
via PPOEnv's existing initial_squad/initial_bank/initial_free_transfers
constructor arguments.

TRANSFERS IN/OUT are a plain set-difference over the squad_ids the MILP
already returned each step (info["squad_ids"]) versus the previous
gameweek's - not a second transfer calculation, since select_squad() itself
only reports a transfer *count* (info["transfers"]), never the specific
player IDs.
...
Resume later from a saved state:
...
@dataclass
class SeasonState:
    """Everything needed to resume a sequential run - deliberately just the
    4 fields PPOEnv's constructor already accepts (start_gameweek,
    initial_squad, initial_bank, initial_free_transfers), nothing more."""
    gameweek: int
