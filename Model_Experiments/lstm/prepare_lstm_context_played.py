...
OUTPUT_DIR = BASE_DIR / "data" / "lstm_context_played_prepared"
...
# Train only on targets where the player actually played
train_mask = (
    (metadata_df["target_gw"] <= 29)
    & (metadata_df["target_minutes"] > 0)
)

# Keep validation and test unchanged for a fair comparison
val_mask = (
...
print("0-minute training targets:", (train_metadata["target_minutes"] == 0).sum())
